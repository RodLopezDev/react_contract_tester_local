...
interface UploadABIFormProps {
  isLoading: boolean;
  onChange: (abi: any, address: string) => void;
}

const UploadABIForm: FC<UploadABIFormProps> = ({ onChange, isLoading }) => {
  ...
  const onSubmit = (data: IUploadABIForm) => {
    ...
    onChange(data.abi, data.address);
  };
